...
// 如果callbackBlocks count 为0，就会执行cacel操作
...
// 1.移除callbackBlocks的所有元素
// 2.dataTask 置为nil
// 3.把执行请求的数据置为nil,这里需要获取执行Task的Queue,在该Queue里面进行nil操作,不然引起错误
// 4.如果有ownedSession，那么执行 invalidateAndCancel 和 置为 nil
- (void)reset {
    __weak typeof(self) weakSelf = self;
    dispatch_barrier_async(self.barrierQueue, ^{
